# =============================================================================
# Wagtail Pipeline v0.15 — GreenGenes2 variant
# 16S amplicon sequencing analysis pipeline (Illumina single-end only)
# Reference database: GreenGenes2 (gg2_2024_full.mmi + 2024.09.taxonomy.id.tsv)
# =============================================================================
...
rule mappy:
    ...
    input:
        fasta = f"{RUN}/4_rep_seqs_wagtail/{{filename}}/dna-sequences.fasta",
        ref   = f"{db_dir}/gg2_2024_full.mmi"
    output:
...
rule extract_taxonomy:
    ...
    GG2 variant: requires an extra taxonomy ID mapping file (2024.09.taxonomy.id.tsv)
    to resolve GreenGenes2 numeric IDs to taxon names.
    """
    group: "wagtail"
    input:
        ...
        tax_id     = f"{db_dir}/2024.09.taxonomy.id.tsv"
    output:
