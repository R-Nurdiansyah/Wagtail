#snakemake file to benchmark scalability qiime2 pipeline, only until quality control
#used tool here is qiime2 PAIRED END, DEBLUR, MINIMAP and delete the vsearch merge-pairs
...
filenames = [line.strip() for line in open(f"{data_dir}/zymo.tsv")]
...
#need script to create manifest file??
    input:
        #can we put user defined directory here?
        manifest = f"{data_dir}/manifest/{{filename}}_manifest.csv"
    output:
...
rule quality_control:
    #need to check the available parameters for the quality control
    #run the quality control using qiime2 quality-filter q-score
    input: 
        "1_import_combine1/{filename}-single.qza"
    output: 
        filtered = "2_qc_combine1/{filename}-filtered.qza",
        stats = "2_qc_combine1/{filename}-filter-stats.qza"
    conda:
        "envs/qiime2-amplicon-2023.9-py38-linux-conda.yml"
    log:
...
#delete the trunc length as it is only single end data
    ...
    output: 
        ...
        stats = "3_deblur_combine/{filename}-stats.qza"
    conda:
        "envs/qiime2-amplicon-2023.9-py38-linux-conda.yml"
    log:
...
#export the biom table from the abundance table using the custom script
    input: 
        artifact = "3_deblur_combine/{filename}-rep-seqs.qza"
    output:
    ...
    resources:
        ...
        runtime = "96h"
    shell:
        "(python {params.script} --input-path {input.artifact} "
        "--output-path {output.output}) 2> {log}"

rule map_minimap2:
#use mappy script in version 0.02 -> see if it is increase efficiency
    input:
    ...
    output:
        "5_taxonomy_combine/{filename}.sam"
    conda:
        "envs/minimap2.yaml"
    log:
        "0_logs_combine/{filename}/map_minimap2.log"
        #log and benchmark files are located in 0_logs folder and the subdirectory with user-defined name
    benchmark:
        "0_logs_combine/{filename}/map_minimap2.benchmark.txt"
    threads:
        1
    resources:
        mem_mb = 16000,
        runtime = "96h"
    shell:
        "(minimap2 -ax sr -t {threads} {input.ref} {input.F} > {output}) 2> {log}"

rule extract_primary:
#extract primary ID from the sam file for input in extract_taxonomy.py script
    input: 
        "5_taxonomy_combine/{filename}.sam"
    output: 
        "5_taxonomy_combine/{filename}_primary.tsv"
    conda:
        "envs/minimap2.yaml"
    log:
        "0_logs_combine/{filename}/extract_primary.log"
    benchmark:
        "0_logs_combine/{filename}/extract_primary.benchmark.txt"
    threads:
        1
    resources:
        mem_mb = 16000,
        runtime = "96h"
    shell:
        "(samtools view -F260 {input} | cut -f 1,3 > {output}) 2> {log}"
...
#to edit the abundance tsv file for input in the next script, take all from 3rd lines
    input: 
        "4_table_combine/{filename}_table.tsv"
    output: 
        "5_taxonomy_combine/{filename}_table_edit.tsv"
    conda:
        "envs/qiime2-amplicon-2023.9-py38-linux-conda.yml"
    log:
...
#extract taxonomy data based on the database and format the output for filling the taxonomy
    input:
        primary = "5_taxonomy_combine/{filename}_primary.tsv",
        table = "5_taxonomy_combine/{filename}_table_edit.tsv"
    output: 
